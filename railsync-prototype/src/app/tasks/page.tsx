                  ...
                  {filtered.map((task, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-xs text-[#526175]">{task.source}</td>
                        ...
                        <td className="px-4 py-3 text-xs text-[#526175]">{task.corridorName}</td>
                        <td className="px-4 py-3 text-xs font-medium text-[#0F172A]">{task.title}</td>
                        <td className="px-4 py-3 text-xs text-[#526175]">{formatDuration(task.durationMin)}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5">
                            <span className="text-xs font-bold" style={{ color: priorityColors[task.priorityBand] }}>{task.priorityScore}</span>
                            <Badge className="text-xs px-1.5 py-0" style={{ backgroundColor: `${priorityColors[task.priorityBand]}15`, color: priorityColors[task.priorityBand], border: `1px solid ${priorityColors[task.priorityBand]}30` }}>
                              {task.priorityBand}
                            </Badge>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <Badge variant="outline" className={`text-xs ${dueStatus === 'overdue' ? 'text-red-600 border-red-300 bg-red-50' : dueStatus === 'today' ? 'text-amber-800 border-amber-300 bg-amber-50' : 'text-slate-500 border-slate-300'}`}>
                            {getDueStatusLabel(task.dueDate)}
                          </Badge>
                        </td>
                        <td className="px-4 py-3 text-xs text-[#526175]">{task.requiredState}</td>
                      </motion.tr>
                    );
                  })}
                </tbody>
            ...
            <>
              ...
              <div className="mt-4 space-y-4">
                <div>
                  ...
                  <p className="text-sm text-[#526175] mt-1">{selectedTask.corridorName} • {selectedTask.source}</p>
                </div>
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={item.label} className="flex items-center justify-between">
                        <span className="text-xs text-[#526175]">{item.label}</span>
